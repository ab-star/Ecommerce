...
import { controller, httpPost, requestBody, response, request, httpGet, next } from 'inversify-express-utils';
...
export class SalesOrderController {
  ...
  public async createSalesOrder(@requestBody() salesOrderData: any,
   @response() res: any , @next() next: NextFunction) {
    try {
      const result = await this.salesOrderService.createSalesOrder(salesOrderData);
      res.status(201).json(new ApiResponse(true, 'Sales order created successfully', result));
    } catch (error: unknown) {
      next(error)
    }
  }
  ...
  @httpGet('/')
  public async getSalesOrders(@request() req: any, @response() res: any , @next() next: NextFunction) {
